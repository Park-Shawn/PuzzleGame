...
import ranking.Properties;
import ranking.RankingGraphic;
import ranking.Reading;
...
class Puzzle implements ActionListener, WindowListener {
	...
	void InArray() {
		...
		while (true) {
			...
			for (int i = 0; i < 100; i++) {
				int num = 0;
				int rnum = (int) (Math.random() * ((size * size) - 2)); // rnum에 맨 마지막 번지의 값을 제외한 나머지 숫자를 랜덤으로 뽑는다.
